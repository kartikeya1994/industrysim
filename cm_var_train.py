...
nn = NN(dim_input=state_size, dim_hidden_layers=nn_arch, dim_output=action_size, do_dropout=True, filename='cm_var.pickle')
...
for mt_fixed_cost in mt_fixed_cost_list:
	...
	for exp in range(validation):
		...
		for mr in res.machine_results:
			...
			cm_jobs[mr.name] += mr.mt_jobs_done['cm']
