...
def thread_task(t):
	epoch_result, state = t[0].run_epoch(Policy('SJF', t[1]))
...
nn = NN(dim_input=state_size, dim_hidden_layers=[10,10,10], dim_output=action_size, do_dropout=True)
...
for exp in range(training_passes):
	...
	for i in range(max_epochs):
		...
		#par = [(NN.clone(env), pm_plan), (NN.clone(env), pm_plan)]#, (NN.clone(env), pm_plan)]
